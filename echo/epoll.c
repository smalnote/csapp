/*
 * Ref: https://www.10k.cn/article/33948.html
 */
#include <arpa/inet.h>
#include <errno.h>
#include <fcntl.h>
...
int main(int argc, char *argv[]) {
    ...
    socketfd = create_and_bind(argv[1]);
    if (socketfd == -1) abort();
    ...
    s = epoll_ctl(efd, EPOLL_CTL_ADD, socketfd, &event);
    if (s == -1) {
        perror("epoll_ctl");
        abort();
    }
        ...
        for (i = 0; i < n; i++) {
            if ((events[i].events & EPOLLERR) || (events[i].events & EPOLLHUP) ||
                ...
                while (1) {
                    ...
                    incomingfd = accept(socketfd, &in_addr, &in_len);
                    if (incomingfd == -1) {
                    ...
                    s = make_socket_non_blocking(incomingfd);
                    if (s == -1) abort();

                    event.data.fd = incomingfd;
                    event.events = EPOLLIN | EPOLLET;
                    s = epoll_ctl(efd, EPOLL_CTL_ADD, incomingfd, &event);
                    if (s == -1) {
                        perror("epoll_ctl");
                        abort();
                    }
